            ...
            ->children()
                ...
                ->arrayNode('accounting')
                    ...
                    ->children()
                        ...
                        ->end()
                        ->scalarNode('bank_account')
                            ->defaultValue('512000')
                            ->info('Bank account (French PCG)')
                        ->end()
                        ->scalarNode('bank_journal_code')
                            ->defaultValue('BQ')
                            ->info('Journal code for bank (e.g., BQ for "Banque")')
                        ->end()
                        ->scalarNode('bank_journal_label')
                            ->defaultValue('Banque')
                            ->info('Journal label for bank')
                        ->end()
                    ->end()
                ->end()
                ...
                ->arrayNode('factur_x')
                    ...
                    ->children()
                        ...
                        ->end()
                        ->booleanNode('validate_xml')
                            ->defaultTrue()
                            ->info('Validate XML before embedding in PDF (recommended)')
                        ->end()
                    ->end()
                ->end()

                // PDP (Plateforme de Dématérialisation Partenaire) configuration
                ->arrayNode('pdp')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->booleanNode('enabled')
                            ->defaultFalse()
                            ->info('Enable PDP integration for e-invoicing')
                        ->end()
                        ->scalarNode('default_connector')
                            ->defaultNull()
                            ->info('Default PDP connector ID (e.g., pennylane, chorus_pro)')
                        ->end()
                        ->booleanNode('auto_send_on_finalize')
                            ->defaultFalse()
                            ->info('Automatically send invoice to PDP after finalization')
                        ->end()
                    ->end()
                ->end()
                ...
                ->arrayNode('company')
                    ...
                    ->children()
                        ...
                        ->scalarNode('city')
                            ->defaultNull()
                            ->info('City name (EN16931 BT-37)')
                        ->end()
                        ->scalarNode('postal_code')
                            ->defaultNull()
                            ->info('Postal code (EN16931 BT-38)')
                        ->end()
                        ->scalarNode('country_code')
                            ->defaultValue('FR')
                            ->info('ISO 3166-1 alpha-2 country code (EN16931 BT-40)')
                        ->end()
                    ->end()
                ->end()
